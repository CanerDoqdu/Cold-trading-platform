...
function ServiceCard({ icon, title, href }: { icon: string; title: string; href: string }) {
  return (
    <Link 
      href={href}
      className="group flex flex-col items-center p-6 bg-gradient-to-br from-gray-100 dark:from-[#1a1f2e]/50 to-transparent rounded-xl border border-gray-200 dark:border-gray-800/30 hover:border-purple-500/30 transition-all duration-300 hover:bg-gray-100 dark:hover:bg-[#1a1f2e]/80"
    >
      ...
      <h3 className="text-gray-900 dark:text-white text-sm font-medium text-center leading-tight group-hover:text-purple-400 transition-colors">
        {title}
      </h3>
    </Link>
  );
...
function FAQItem({ question, answer, isOpen, onClick }: { 
  ...
  return (
    <div className="border-b border-gray-200 dark:border-gray-800/50">
      ...
      >
        <h3 className="text-gray-900 dark:text-white font-semibold text-base group-hover:text-purple-400 transition-colors">
          {question}
        </h3>
        <svg 
...
export default function SupportPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0d14] pt-24 pb-16">
      {/* Header */}
        ...
        <form onSubmit={handleSearch} className="flex gap-3 max-w-xl">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white dark:bg-[#1a1f2e] border border-gray-300 dark:border-gray-700/50 rounded-lg text-gray-900 dark:text-white text-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-purple-500/50 transition-colors"
            />
          </div>
          <button 
            type="submit"
            className="px-6 py-3 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-700/50 transition-colors"
          >
            Search
          </button>
        </form>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 mb-16">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-8">Self-Service</h2>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="bg-gradient-to-br from-gray-100 dark:from-[#1a1f2e]/80 to-white dark:to-[#0f1318] rounded-2xl border border-gray-200 dark:border-gray-800/50 p-6 md:p-8">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 mt-16">
        <div className="bg-gradient-to-r from-purple-500/10 to-cyan-500/10 rounded-2xl border border-purple-500/20 p-8 md:p-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button 
              onClick={() => window.dispatchEvent(new Event('openLivaChat'))}
              className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg transition-colors inline-flex items-center justify-center gap-2"
            >
            ...
            <button 
              onClick={() => {
                alert('Ticket submission feature coming soon! For now, please use Live Chat for immediate assistance.');
              }}
              className="px-8 py-3 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-900 dark:text-white font-medium rounded-lg border border-gray-300 dark:border-gray-700/50 transition-colors inline-flex items-center justify-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
