...
export const metadata: Metadata = {
  title: 'Crypto News',
  description: 'Stay updated with the latest cryptocurrency news, market analysis, and blockchain updates from top sources like CoinDesk and Cointelegraph.',
};
...
function NewsCard({ article }: { article: NewsArticle }) {
  return (
    <Link 
      ...
      className="group flex gap-4 p-4 bg-gradient-to-r from-gray-100 dark:from-[#1a1f2e] to-white dark:to-[#0f1318] rounded-xl border border-gray-200 dark:border-gray-800/50 hover:border-cyan-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/5"
    >
      {/* Small Thumbnail */}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-cyan-600 dark:text-cyan-400 text-xs font-medium">{article.sourceName}</span>
            <span className="text-gray-400 dark:text-gray-600 text-xs">•</span>
            <span className="text-gray-500 text-xs">{formatTimeAgo(article.publishedOn)}</span>
          </div>
          <h3 className="text-gray-900 dark:text-white font-semibold text-sm leading-tight group-hover:text-cyan-400 transition-colors line-clamp-2">
            {article.title}
          </h3>
        </div>
      </div>
...
function TextNewsItem({ article, index }: { article: NewsArticle; index: number }) {
  return (
    <Link 
      ...
      target="_blank" 
      rel="noopener noreferrer"
      className="group flex items-start gap-3 py-3 border-b border-gray-200 dark:border-gray-800/50 last:border-0"
    >
      <span className="text-xl font-bold text-gray-400 dark:text-gray-700 group-hover:text-emerald-400 transition-colors flex-shrink-0 w-7">
        {String(index + 1).padStart(2, '0')}
      </span>
      <div className="flex-1 min-w-0">
        <h3 className="text-gray-900 dark:text-white text-sm font-medium leading-tight group-hover:text-emerald-400 transition-colors line-clamp-2">
          {article.title}
        </h3>
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-gray-600 dark:text-gray-400 text-xs">{article.sourceName}</span>
          <span className="text-gray-400 dark:text-gray-600 text-xs">•</span>
          <span className="text-gray-600 dark:text-gray-400 text-xs">{formatTimeAgo(article.publishedOn)}</span>
        </div>
      </div>
    </Link>
  );
...
function NewsCardSkeleton() {
  return (
    <div className="flex gap-4 p-4 bg-gradient-to-r from-gray-100 dark:from-[#1a1f2e] to-white dark:to-[#0f1318] rounded-xl border border-gray-200 dark:border-gray-800/50 animate-pulse">
      <div className="w-20 h-20 bg-gray-300 dark:bg-gray-700 rounded-lg flex-shrink-0" />
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              ...
              <div className="bg-gradient-to-br from-gray-100 dark:from-[#1a1f2e] to-white dark:to-[#0f1318] rounded-2xl border border-gray-200 dark:border-gray-800/50 p-5 mb-6">
              ...
              <div className="bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 rounded-2xl border border-emerald-500/20 p-6">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Stay Informed</h3>
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">Get the latest crypto news delivered to your inbox.</p>
                <div className="flex gap-2">
                  <input 
                    type="email" 
                    placeholder="Enter your email"
                    aria-label="Email address for newsletter"
                    className="flex-1 bg-white dark:bg-black/30 border border-gray-300 dark:border-gray-700 rounded-lg px-4 py-2 text-gray-900 dark:text-white text-sm placeholder-gray-500 focus:outline-none focus:border-emerald-500/50"
                  />
...
export default function NewsPage() {
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0d14] pt-24 pb-16">
      {/* Hero Header */}
